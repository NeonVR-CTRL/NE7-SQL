...
.db-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:16px}.db-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:20px;cursor:pointer;transition:transform .3s var(--ease),border-color .3s,box-shadow .3s;position:relative;overflow:hidden;backdrop-filter:blur(20px);transform-style:preserve-3d;-webkit-tap-highlight-color:transparent}.card-enter{animation:cardIn .6s var(--ease-out)}@keyframes cardIn{from{opacity:0;transform:translateY(24px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}}.db-card::before{content:'';position:absolute;inset:0;background:radial-gradient(400px circle at var(--mx,50%) var(--my,50%),rgba(52,211,153,.07),transparent 45%);opacity:0;transition:opacity .4s;pointer-events:none}.db-card:hover{border-color:var(--accent);box-shadow:0 0 0 1px var(--accent),0 8px 32px var(--accentGlow)}.db-card:hover::before{opacity:1}.db-card > *{position:relative;z-index:2}.db-card-top{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;gap:8px}.db-name{font-weight:700;font-size:1rem;font-family:'JetBrains Mono',monospace;letter-spacing:-.01em;word-break:break-all}.db-status{font-size:.6rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;padding:3px 8px;border-radius:4px;display:flex;align-items:center;gap:5px;flex-shrink:0}.db-status.active{background:rgba(52,211,153,.1);color:var(--green)}.db-status.active::before{content:'';width:5px;height:5px;border-radius:50%;background:var(--green);animation:pul 2s infinite}.db-status.unassigned{background:rgba(251,191,36,.1);color:var(--amber)}
.db-creds{background:rgba(0,0,0,.3);border:1px solid var(--border);border-radius:8px;padding:10px 12px;margin:12px 0;font-family:'JetBrains Mono',monospace;font-size:.72rem;color:var(--text2);line-height:1.7}.db-creds b{color:var(--accent);font-weight:600;margin-right:6px}.db-creds .copy-inline{background:var(--surface3);border:none;color:var(--text2);padding:2px 6px;border-radius:4px;font-size:.6rem;cursor:pointer;margin-left:6px}.db-creds .copy-inline:hover{color:var(--accent)}
.db-meta{display:flex;gap:16px;font-size:.72rem;color:var(--text3);margin-bottom:12px;flex-wrap:wrap}.db-meta span{display:flex;align-items:center;gap:4px}.db-meta svg{width:12px;height:12px;stroke:currentColor;fill:none;stroke-width:2}
.db-storage-bar{height:6px;background:var(--surface3);border-radius:3px;overflow:hidden;margin:8px 0 4px}.db-storage-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,var(--accent),var(--accent2));transition:width .8s var(--ease-out);position:relative;overflow:hidden}.db-storage-fill::after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,.3),transparent);animation:shim 2.5s infinite}@keyframes shim{0%{transform:translateX(-100%)}100%{transform:translateX(100%)}}.db-storage-text{display:flex;justify-content:space-between;font-size:.65rem;color:var(--text3)}
...
.info-box{background:rgba(129,140,248,.08);border:1px solid rgba(129,140,248,.25);border-radius:10px;padding:12px 14px;font-size:.78rem;color:var(--text2);margin-bottom:18px;line-height:1.6}
.assignee-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:20px;font-size:.68rem;font-weight:600;background:rgba(56,189,248,.1);color:var(--accent3);margin-top:4px}
@media(max-width:1200px){.span-3{grid-column:span 6}.span-4{grid-column:span 6}.span-8{grid-column:span 12}}@media(max-width:900px){.mobile-topbar{display:flex}.sidebar{transform:translateX(-100%);width:260px;padding-top:72px}.sidebar.open{transform:translateX(0);box-shadow:20px 0 60px rgba(0,0,0,.5)}.main{margin-left:0;padding:72px 16px 32px}.span-3,.span-4,.span-6,.span-8{grid-column:span 12}}@media(max-width:640px){html{font-size:14px}.main{padding:68px 12px 24px}.bento,.db-grid{gap:12px}.card{padding:18px}.stat-value{font-size:1.6rem}.header h1{font-size:1.35rem}.header-actions{width:100%}.header-actions .btn{flex:1;justify-content:center}.modal{padding:24px;border-radius:16px}.toast-container{left:12px;right:12px;bottom:12px}.toast{min-width:auto;width:100%}.terminal-body{height:260px;font-size:.7rem}}
</style></head><body>
...
<div class="logo"><div class="logo-mark"><svg viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg></div><div class="logo-text">NE7-SQL<span>Enterprise</span></div></div>
<div class="nav-section">
<div class="nav-label">Platform</div>
<div class="nav-item admin-only" data-view="overview"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/></svg>Overview</div>
<div class="nav-item" data-view="databases"><svg viewBox="0 0 24 24"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg><span id="nav-db-label">My Databases</span><span class="nav-badge" id="nav-db-count">0</span></div>
<div class="nav-item admin-only" data-view="keys"><svg viewBox="0 0 24 24"><path d="m21 2-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0 3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>API Keys<span class="nav-badge" id="nav-key-count">0</span></div>
<div class="nav-item admin-only" data-view="admins"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>Admins</div>
<div class="nav-item admin-only" data-view="customers"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Customers</div>
</div>
<div class="nav-section">
<div class="nav-label">Tools</div>
<div class="nav-item" data-view="console"><svg viewBox="0 0 24 24"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>SQL Console</div>
<div class="nav-item admin-only" data-view="panel"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>NE7 Panel</div>
</div>
<div class="nav-section">
<div class="nav-label">System</div>
<div class="nav-item admin-only" data-view="settings"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>Settings</div>
<div class="nav-item" id="logout" style="color:var(--red)"><svg viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Logout</div>
</div>
<div class="sidebar-footer"><div class="status-pill"><span class="status-dot"></span>All systems operational</div></div>
</aside>

<main class="main">
<div id="view-overview"><div class="header"><h1>Overview</h1><div class="header-actions"><button class="btn btn-primary" id="new-db-btn-ov"><svg viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Create Database</button></div></div><div class="bento"><div class="card span-3 stat-card"><div class="stat-header"><div class="stat-icon purple"><svg viewBox="0 0 24 24"><path d="m21 2-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0 3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg></div></div><div><div class="stat-value" id="stat-keys">0</div><div class="stat-label">API Keys</div></div></div><div class="card span-3 stat-card"><div class="stat-header"><div class="stat-icon green"><svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg></div></div><div><div class="stat-value"><span id="stat-total">0</span><span style="font-size:1rem;color:var(--text3)"> GB</span></div><div class="stat-label">Grand Total</div></div></div><div class="card span-3 stat-card"><div class="stat-header"><div class="stat-icon amber"><svg viewBox="0 0 24 24"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/></svg></div></div><div><div class="stat-value"><span id="stat-used">0</span><span style="font-size:1rem;color:var(--text3)"> GB</span></div><div class="stat-label">Grand Used</div></div></div><div class="card span-3 stat-card"><div class="stat-header"><div class="stat-icon red"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg></div></div><div><div class="stat-value"><span id="stat-balance">0</span><span style="font-size:1rem;color:var(--text3)"> GB</span></div><div class="stat-label">Grand Balance</div></div></div><div class="card span-12"><div class="section-header"><div class="section-title">Storage per API Key</div></div><div class="table-wrap"><table><thead><tr><th>Key</th><th>Status</th><th>Capacity</th><th>Used</th><th>Balance</th></tr></thead><tbody id="overview-storage-body"></tbody></table></div></div></div></div>

<div id="view-databases" style="display:none"><div class="header"><h1 id="db-title">My Databases</h1><div class="header-actions"><button class="btn btn-primary admin-only" id="new-db-btn"><svg viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Create Database</button></div></div><div class="db-grid" id="db-grid"></div></div>
...
<div id="view-customers" style="display:none"><div class="header"><h1>Customers</h1><div class="header-actions"><button class="btn btn-primary" id="add-cust-btn"><svg viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Add Customer</button></div></div><div class="info-box">Customers can sign in and access only the databases you assign to them. They cannot create new databases.</div><div class="table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Tenant</th><th>DBs Assigned</th><th>Actions</th></tr></thead><tbody id="cust-table"></tbody></table></div></div>

<div id="view-console" style="display:none"><div class="header"><h1>SQL Console</h1></div><div class="terminal"><div class="terminal-header"><div class="terminal-dots"><span></span><span></span><span></span></div><div class="terminal-title" id="console-title">psql — ne7sql</div></div><div class="terminal-body" id="console-output" style="height:400px"><div class="log-line"><span class="log-msg" style="color:var(--green)">Connected to NE7-SQL Enterprise</span></div></div><div style="padding:12px 16px;border-top:1px solid var(--border)"><div class="sql-input-wrap"><span class="sql-prompt">ne7sql=#</span><input class="sql-input" id="sql-db" placeholder="tenant (auto)" style="max-width:140px"><input class="sql-input" id="sql-input" placeholder="SELECT * FROM users..." autocomplete="off"><button class="btn btn-primary btn-sm" id="sql-run">Run</button></div></div></div></div>
...
<div class="modal-overlay" id="modal-db"><div class="modal"><h2>Create Isolated Database</h2><p>Each database is a fully isolated tenant with its own credentials and storage quota.</p><div class="form-group"><label class="form-label">Database Nickname</label><input class="form-input" id="db-name" placeholder="acme-analytics-prod"></div><div class="form-group"><label class="form-label">Initial Storage <span class="hint">(GB, up to 20 per key)</span></label><input class="form-input" id="db-size" type="number" value="5" min="1" max="20"></div><div class="form-group"><label class="form-label">Assign to Customer <span class="hint">(optional)</span></label><select class="form-input" id="db-assign"><option value="">— Unassigned (keep in admin pool) —</option></select></div><div class="form-group"><label class="form-label">Initial Password <span class="hint">(auto-generated if blank)</span></label><input class="form-input" id="db-pass" type="text" placeholder="Leave empty for random"></div><div class="modal-actions"><button class="btn btn-ghost" data-close>Cancel</button><button class="btn btn-primary" id="create-db">Create + Provision</button></div></div></div>

<div class="modal-overlay" id="modal-topup"><div class="modal"><h2>Top Up Storage</h2><p>Increase the storage quota for this database.</p><div class="form-group"><label class="form-label">Database</label><input class="form-input" id="topup-name" disabled></div><div class="form-group"><label class="form-label">Add Storage (GB)</label><input class="form-input" id="topup-amount" type="number" value="5" min="1" max="20"></div><div class="modal-actions"><button class="btn btn-ghost" data-close>Cancel</button><button class="btn btn-primary" id="topup-confirm">Top Up</button></div></div></div>

<div class="modal-overlay" id="modal-assign"><div class="modal"><h2>Assign Database</h2><p>Gift or assign this database to a customer.</p><div class="form-group"><label class="form-label">Database</label><input class="form-input" id="assign-db-name" disabled></div><div class="form-group"><label class="form-label">Assign to Customer</label><select class="form-input" id="assign-customer"></select></div><div class="modal-actions"><button class="btn btn-ghost" data-close>Cancel</button><button class="btn btn-primary" id="assign-confirm">Assign</button></div></div></div>
...
function toast(msg, type){
  ...
  c.appendChild(t); setTimeout(function(){ t.style.opacity='0'; t.style.transform='translateX(60px)'; t.style.transition='all .4s var(--ease)'; setTimeout(function(){ t.remove(); }, 400); }, 4500);
}
function copyText(text, label){
  if(navigator.clipboard && window.isSecureContext){ navigator.clipboard.writeText(text).then(function(){ toast((label||'Value') + ' copied'); }); }
  else { var ta = document.createElement('textarea'); ta.value = text; ta.style.position='fixed'; ta.style.opacity='0'; document.body.appendChild(ta); ta.focus(); ta.select(); try { document.execCommand('copy'); toast((label||'Value') + ' copied'); } catch(e){} document.body.removeChild(ta); }
}
function genPassword(){ var c = 'abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789!@#$'; var p = ''; for (var i=0;i<16;i++) p += c.charAt(Math.floor(Math.random()*c.length)); return p; }
...
api('/api/status').then(function(st){
  if (st && st.setup === true) {
    api('/api/me').then(function(me){
      if (me && me.role) { ROLE = me.role; NAME = me.name || me.email; TENANT = me.tenantId || ''; EMAIL = me.email; enterApp(); }
      else { showLogin(); }
    });
...
$('#su-go').addEventListener('click', function(){
  var btn = $('#su-go'); btn.disabled = true; btn.textContent = 'Initializing...';
...
$('#li-go').addEventListener('click', function(){
  var btn = $('#li-go'); btn.disabled = true; btn.textContent = 'Signing in...';
  api('/api/login','POST',{email:$('#li-email').value, password:$('#li-pass').value}).then(function(r){
    btn.disabled = false; btn.textContent = 'Sign In';
    if (r.token) { TOKEN = r.token; ROLE = r.role; NAME = r.name || r.email; TENANT = r.tenantId || ''; EMAIL = r.email; localStorage.setItem('ne7_token', TOKEN); enterApp(); }
...
function enterApp(){
  ...
  if (ROLE === 'admin') {
    $('#nav-db-label').textContent = 'All Databases';
    $('#db-title').textContent = 'All Databases';
  } else {
    $('#nav-db-label').textContent = 'My Databases';
    $('#db-title').textContent = 'My Databases';
  }
...
function loadOverview(){
  ...
  api('/api/admin/storage').then(function(s){
    ...
    $('#overview-storage-body').innerHTML = s.rows.map(function(r){
...
function loadDatabases(){
  api('/api/databases').then(function(dbs){
    cachedDatabases = dbs || [];
    $('#nav-db-count').textContent = cachedDatabases.length;
    renderDatabases(cachedDatabases);
  });
}
...
function renderDatabases(dbs){
  ...
  // Filter for customers: only show DBs where tenantId matches
  if (ROLE !== 'admin') {
    dbs = dbs.filter(function(db){ return db.tenantId === TENANT || db.tenant === TENANT || db.name === TENANT; });
  }
  
  if(!dbs.length){ 
    grid.innerHTML = '<div class="empty-state"><svg viewBox="0 0 24 24"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg><h3>' + (ROLE === 'admin' ? 'No databases yet' : 'No databases assigned to you') + '</h3><p>' + (ROLE === 'admin' ? 'Create your first isolated database to get started.' : 'Contact your administrator to get a database assigned.') + '</p>' + (ROLE === 'admin' ? '<button class="btn btn-primary" onclick="document.getElementById(\'new-db-btn\').click()">Create Database</button>' : '') + '</div>'; 
    return; 
  }
  ...
  dbs.forEach(function(db){
    ...
    var tenantId = db.tenantId || db.tenant || db.name;
    var usedMB = db.usedMB || 0;
    var maxMB = db.maxSizeMB || (db.capacityGB ? db.capacityGB * 1024 : 5120);
    var pct = Math.min((usedMB / maxMB) * 100, 100);
    var tables = db.tables || 0;
    var key = db.key || '—';
    
    // Admin sees assignee badge; customer doesn't need it
    var assigneeHtml = '';
    if (ROLE === 'admin' && db.ownerEmail) {
      assigneeHtml = '<div class="assignee-badge">👤 Assigned: ' + db.ownerEmail + '</div>';
    } else if (ROLE === 'admin' && !db.ownerEmail) {
      assigneeHtml = '<div class="assignee-badge" style="background:rgba(251,191,36,.1);color:var(--amber)">⚠ Unassigned</div>';
    }
    
    // Credentials display (only show for owner or admin)
    var credsHtml = '';
    if (ROLE === 'admin' || db.tenantId === TENANT) {
      var dsn = db.dsn || ('postgresql://' + tenantId + ':***@ne7-sql.pyypl2005-4b6.workers.dev:5432/' + tenantId);
      credsHtml = '<div class="db-creds">' +
        '<div><b>DSN:</b><span style="word-break:break-all">' + dsn + '</span><button class="copy-inline" data-copy="' + dsn.replace(/"/g,'&quot;') + '">Copy</button></div>' +
    ...
    if (ROLE === 'admin') {
      actionsHtml = '<div class="db-actions">' +
        '<button class="btn btn-amber btn-sm" data-act="topup" data-id="' + tenantId + '">⬆ Top Up</button>' +
        '<button class="btn btn-ghost btn-sm" data-act="assign" data-id="' + tenantId + '">👤 Assign</button>' +
        '<button class="btn btn-danger btn-sm" data-act="delete" data-id="' + tenantId + '" data-name="' + db.name + '">Delete</button>' +
        '</div>';
    } else {
      actionsHtml = '<div class="db-actions">' +
        '<button class="btn btn-ghost btn-sm" data-act="use" data-id="' + tenantId + '">Use in Console</button>' +
        '</div>';
    }
    
    card.innerHTML = '<div class="db-card-top"><div class="db-name">' + db.name + '</div><div class="db-status active">Active</div></div>' +
      assigneeHtml +
      '<div class="db-meta"><span><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>' + tables + ' tables</span>' +
      '<span><svg viewBox="0 0 24 24"><path d="m21 2-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777z"/></svg>' + key + '</span></div>' +
      credsHtml +
      '<div class="db-storage-bar"><div class="db-storage-fill" style="width:' + pct + '%"></div></div>' +
      '<div class="db-storage-text"><span>' + (usedMB/1024).toFixed(2) + ' GB used</span><span>' + (maxMB/1024).toFixed(1) + ' GB limit</span></div>' +
      actionsHtml;
  ...
  $$('.copy-inline').forEach(function(b){
    b.addEventListener('click', function(e){
      e.stopPropagation();
      copyText(b.getAttribute('data-copy'), 'DSN');
    });
  });
  
  // Bind action buttons
  $$('.db-card [data-act]').forEach(function(btn){
    btn.addEventListener('click', function(e){
      e.stopPropagation();
      ...
      if (act === 'delete') {
        showConfirm('Delete "' + name + '"?', 'This will permanently remove the database and all its data from Drime.', function(){
          api('/api/admin/customers/' + id, 'DELETE').then(function(r){
            if (r && r.ok !== false) { toast('Database deleted'); loadDatabases(); }
            else { toast((r && r.error) || 'Failed to delete', 'error'); }
          });
        });
      } else if (act === 'topup') {
        openTopupModal(id);
...
function openTopupModal(tenantId){
  var db = cachedDatabases.find(function(d){ return (d.tenantId || d.tenant || d.name) === tenantId; });
  if (!db) return;
  $('#topup-name').value = db.name;
  $('#topup-amount').value = 5;
  $('#topup-confirm').onclick = function(){
    var amt = parseInt($('#topup-amount').value) || 5;
    api('/api/admin/databases/' + encodeURIComponent(tenantId) + '/topup', 'POST', { amountGB: amt }).then(function(r){
      if (r && r.ok) { toast('Topped up ' + amt + ' GB'); closeModal('modal-topup'); loadDatabases(); }
      else { toast((r && r.error) || 'Top-up failed. Endpoint may need to be added to backend.', 'error'); }
    });
  };
  openModal('modal-topup');
}

function openAssignModal(tenantId){
  var db = cachedDatabases.find(function(d){ return (d.tenantId || d.tenant || d.name) === tenantId; });
  if (!db) return;
  $('#assign-db-name').value = db.name;
  // Populate customer select
  api('/api/admin/customers').then(function(cs){
    cachedCustomers = cs || [];
    var sel = $('#assign-customer');
    sel.innerHTML = '<option value="">— Unassign (keep in admin pool) —</option>' +
      cachedCustomers.map(function(c){ return '<option value="' + c.id + '">' + c.name + ' (' + c.email + ')</option>'; }).join('');
  });
  $('#assign-confirm').onclick = function(){
    var custId = $('#assign-customer').value;
    api('/api/admin/databases/' + encodeURIComponent(tenantId) + '/assign', 'POST', { customerId: custId }).then(function(r){
      if (r && r.ok) { toast(custId ? 'Database assigned' : 'Database unassigned'); closeModal('modal-assign'); loadDatabases(); }
      else { toast((r && r.error) || 'Assign failed. Endpoint may need to be added to backend.', 'error'); }
    });
  };
...
function loadKeys(){
  api('/api/admin/keys').then(function(ks){
    if (!ks) return;
    $('#nav-key-count').textContent = ks.length;
    var tbody = $('#key-table');
    if(!ks.length){ tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text3);padding:40px">No API keys added yet</td></tr>'; return; }
    tbody.innerHTML = ks.map(function(k){
      return '<tr data-id="' + k.id + '"><td style="font-family:Inter;font-weight:600">' + k.nickname + '</td><td><div class="key-cell"><span class="key-text">' + k.key + '</span></div></td><td>' + k.capacityGB + ' GB</td><td><span class="badge ' + (k.status==='HEALTHY'?'badge-green':'badge-red') + '">' + (k.status || 'UNKNOWN') + '</span></td><td><div class="key-actions"><button class="icon-btn" data-act="eye" title="Show/Hide"><svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button><button class="icon-btn" data-act="copy" title="Copy"><svg viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button><button class="icon-btn danger" data-act="delkey" data-nick="' + k.nickname + '" title="Delete"><svg viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></div></td></tr>';
    }).join('');
    $$('#key-table [data-act]').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        else if(act === 'delkey') showConfirm('Remove "' + btn.getAttribute('data-nick') + '"?', 'This API key will be removed from the storage pool.', function(){ api('/api/admin/keys/' + id, 'DELETE').then(function(){ toast('Key removed'); loadKeys(); }); });
      });
    });
  });
}
...
function toggleKey(id, row){
  ...
  if(textEl.getAttribute('data-revealed')){
  ...
  api('/api/admin/keys/' + id + '/reveal').then(function(res){
    if(res && res.key){
      textEl.setAttribute('data-masked', textEl.textContent);
      textEl.textContent = res.key;
...
function copyKey(id){
  api('/api/admin/keys/' + id + '/reveal').then(function(res){ if(res && res.key){ copyText(res.key, 'API Key'); } });
}
...
function loadAdmins(){
  api('/api/admin/admins').then(function(list){
    ...
    tbody.innerHTML = list.map(function(e){ return '<tr data-email="' + encodeURIComponent(e) + '"><td style="font-family:Inter;font-weight:600">' + e + '</td><td><button class="icon-btn danger" data-act="deladmin"><svg viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></td></tr>'; }).join('');
    $$('#admin-table [data-act]').forEach(function(btn){
      btn.addEventListener('click', function(){
        var row = btn.closest('tr'); var email = row.getAttribute('data-email');
...
function loadCustomers(){
  api('/api/admin/customers').then(function(list){
    ...
    if(!cachedCustomers.length){ tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text3);padding:40px">No customers</td></tr>'; return; }
    // Count DBs per customer
    api('/api/databases').then(function(dbs){
      dbs = dbs || [];
      tbody.innerHTML = cachedCustomers.map(function(c){
        var dbCount = dbs.filter(function(d){ return (d.tenantId || d.tenant || d.name) === c.tenantId; }).length;
        return '<tr data-id="' + c.id + '"><td style="font-family:Inter;font-weight:600">' + c.name + '</td><td>' + c.email + '</td><td style="color:var(--text3)">' + c.tenantId + '</td><td><span class="badge badge-purple">' + dbCount + ' DBs</span></td><td><button class="icon-btn danger" data-act="delcust"><svg viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></td></tr>';
      }).join('');
      $$('#cust-table [data-act]').forEach(function(btn){
        btn.addEventListener('click', function(){
          var row = btn.closest('tr'); var id = row.getAttribute('data-id');
          showConfirm('Delete customer?', 'This removes their access. Their databases will become unassigned.', function(){ api('/api/admin/customers/' + id, 'DELETE').then(function(){ toast('Customer deleted'); loadCustomers(); }); });
        });
      });
    });
  });
  ...
  api('/api/admin/source').then(function(src){
    ...
    $$('#src-list .src-file').forEach(function(f){
      f.addEventListener('click', function(){
        ...
        $('#src-code').textContent = src[f.getAttribute('data-f')];
      });
    });
  });
}

// Create DB handler — admin only
function openCreateDbModal(){
  if (ROLE !== 'admin') { toast('Only admins can create databases', 'error'); return; }
  $('#db-name').value = '';
  $('#db-size').value = 5;
  $('#db-pass').value = '';
  // Populate customer select
  api('/api/admin/customers').then(function(cs){
    cachedCustomers = cs || [];
    var sel = $('#db-assign');
    sel.innerHTML = '<option value="">— Unassigned (keep in admin pool) —</option>' +
      cachedCustomers.map(function(c){ return '<option value="' + c.id + '" data-email="' + c.email + '">' + c.name + ' (' + c.email + ')</option>'; }).join('');
  });
  openModal('modal-db');
}

$('#new-db-btn').addEventListener('click', openCreateDbModal);
$('#new-db-btn-ov').addEventListener('click', openCreateDbModal);
...
$('#create-db').addEventListener('click', function(){
  ...
  if(!name){ toast('Database nickname required','error'); return; }
  var sizeGB = parseInt($('#db-size').value) || 5;
  var password = $('#db-pass').value.trim() || genPassword();
  var assignOpt = $('#db-assign');
  var customerId = assignOpt.value;
  var customerEmail = assignOpt.selectedOptions[0] ? assignOpt.selectedOptions[0].getAttribute('data-email') : '';
  
  var btn = $('#create-db');
  btn.disabled = true; btn.textContent = 'Provisioning...';
  
  // Create the customer (which provisions the DB)
  api('/api/admin/customers','POST',{
    name: name,
    email: customerEmail || (name.toLowerCase().replace(/[^a-z0-9]/g,'') + '@internal.ne7'),
    password: password,
    storageGB: sizeGB
  }).then(function(r){
    btn.disabled = false; btn.textContent = 'Create + Provision';
    if (r && r.token) {
      closeModal('modal-db');
      toast('Database "' + name + '" created! Password: ' + password, 'success');
      // Wait for backend to write manifest, then reload
      setTimeout(function(){ loadDatabases(); }, 1500);
    } else {
      toast((r && r.error) || 'Failed to create database', 'error');
    }
  });
});

$('#add-key').addEventListener('click', function(){
  var nick = $('#key-nick').value.trim(), key = $('#key-val').value.trim();
  if(!nick || !key){ toast('All fields required','error'); return; }
  api('/api/admin/keys','POST',{ nickname:nick, key:key }).then(function(){ closeModal('modal-key'); toast('Key "' + nick + '" added'); $('#key-nick').value=''; $('#key-val').value=''; loadKeys(); loadOverview(); });
});
$('#add-admin').addEventListener('click', function(){
  var email = $('#admin-email').value.trim(), pass = $('#admin-pass').value;
  if(!email || !pass){ toast('All fields required','error'); return; }
...
$('#add-cust').addEventListener('click', function(){
  var name = $('#cust-name').value.trim(), email = $('#cust-email').value.trim(), pass = $('#cust-pass').value;
  if(!name || !email || !pass){ toast('All fields required','error'); return; }
  api('/api/admin/customers','POST',{ name:name, email:email, password:pass }).then(function(r){
    if (r && r.token) {
      closeModal('modal-cust'); toast('Customer created. Assign a DB to them from the Databases view.');
      $('#cust-name').value=''; $('#cust-email').value=''; $('#cust-pass').value=''; loadCustomers();
...
$('#reset-btn').addEventListener('click', function(){
  showConfirm('Confirm Reset', 'Type RESET in the next prompt to wipe everything.', function(){
    ...
    if (w === 'RESET') { api('/api/admin/reset','POST',{}).then(function(r){ toast('Deleted ' + (r.deleted || 0) + ' files.'); loadOverview(); loadDatabases(); }); }
    else { toast('Aborted'); }
  });
});
...
function runSQL(){
  ...
  var tenant = $('#sql-db').value.trim() || TENANT;
  if (!tenant) { toast('No tenant selected','error'); return; }
  
  // Security: customers can ONLY query their own tenant
  if (ROLE !== 'admin' && tenant !== TENANT) {
    toast('Access denied: you can only query your own database','error');
    return;
  }
  
  var out = $('#console-output');
  out.innerHTML += '<div class="log-line"><span class="log-module">' + tenant + '=#</span><span class="log-msg">' + sql + '</span></div>';
  $('#sql-input').value = '';
